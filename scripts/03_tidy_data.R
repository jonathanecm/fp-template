...
# Camacho Jonathan E.
#########################################################
# Performs data cleaning and saving of data.
#########################################################
...
p_values_df <- read.csv("./data/raw/p_values_df.csv", stringsAsFactors = FALSE)

# Joining sets.
tidy_p_values <- left_join(p_values_df, categories_df, by = "abbrev_journal_name") %>%
  select(2:12) %>%
  filter(!is.na(FoR1) & !is.na(FoRCode)) %>% # Changes added for creating CCA data. 
